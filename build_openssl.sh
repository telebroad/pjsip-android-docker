...
if [ "$ANDROID_TARGET_ABI" == "armeabi-v7a" ]; then
    ...
    HOST=$HOST-arm64
elif [ "$ANDROID_TARGET_ABI" == "x86" ] || [ "$ANDROID_TARGET_ABI" == "x86_64" ]; then
    HOST=$HOST-$ANDROID_TARGET_ABI
else
...
./Configure $HOST -D__ANDROID_API__=${ANDROID_TARGET_API} -fPIC no-asm no-shared no-tests --prefix=${OPENSSL_OUTPUT_PATH_TARGET} |
    tee -a /pjsip/build_pjsip.log
build_library |
    tee -a /pjsip/build_pjsip.log
